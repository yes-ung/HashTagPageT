...
//		예외 처리
		...
		SeriesVO insertSeriesVO = new SeriesVO(seriesVO.getCategoryId(),seriesVO.getTypeId(),seriesVO.getSeriesTitle(),seriesVO.getSeriesPrice(),seriesVO.getSeriesDescription(),seriesVO.getThumbnailImgUrl(),seriesVO.getPublisherId(),seriesVO.getAuthorId(),thumbnailData);
...
//		오류검사
	    ...
	    if ("content".equalsIgnoreCase(type)) {
	        imageData = imageCache.getContent(seriesId); // 내용 이미지
	        fileName = seriesVO.getSeriesId() + "_content.jpg";
	    } else {
	        imageData = imageCache.getThumbnail(seriesId); // 기존 썸네일
