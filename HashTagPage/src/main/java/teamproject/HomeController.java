...
public class HomeController {
	...
	public String community1(@ModelAttribute Criteria criteria,
		...
		// 총 페이지 수 계산
		int totalPages = (int) Math.ceil((double) totCnt / criteria.getPageUnit());
		// 현재 페이지 번호가 총 페이지 수보다 크면 마지막 페이지로 보정
		if (criteria.getPageIndex() > totalPages) {
		    criteria.setPageIndex(totalPages);
		    paginationInfo.setCurrentPageNo(totalPages);}
