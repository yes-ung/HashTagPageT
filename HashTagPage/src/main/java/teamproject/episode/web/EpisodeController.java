...
//		예외 처리
	...
	public ResponseEntity<byte[]> fileDownload(@RequestParam(defaultValue = "") String episodeId,
		...
		EpisodeVO episodeVO=episodeService.selectEpisode(episodeId);
		
		byte[] imageData;
	    String fileName;
	    
	    if ("content".equalsIgnoreCase(type)) {
	        imageData = imageCache.getContent("Episode"+episodeId); // 새로 추가된 이미지
	        fileName = episodeVO.getEpisodeId() + "_content.jpg";
	    } else {
	        imageData = imageCache.getThumbnail("Episode"+episodeId); // 기존 썸네일
