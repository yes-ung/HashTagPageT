  ...
  <body>
    ...
    <header class="simple-header mt5 pl2 pr2">
      ...
      <div class="auth-area">
	    ...
	    <a href="<c:url value='/hashtagpage/login.do'/>" class="a-rounded-al login pl2"> 로그인 </a>
        </c:if>
         <!-- {/* 로그아웃 시작 */} -->
        <!-- memberVO 가 세션에 있으면 메뉴을 보이고, 없으면 안보임 -->
        <c:if test="${sessionScope.usersVO != null}">
	     <a href="<c:url value='/hashtagpage/cart.do'/>" class="a-rounded-al cart pl2">장바구니</a>
         <a href="<c:url value='/hashtagpage/mypage.do'/>" class="a-rounded-al wishlist pl2">내정보</a>
         <a href="<c:url value='/logout.do'/>" class="a-rounded-al logout pl2"> 로그아웃</a>
	    </c:if>
        <!-- {/* 로그아웃 끝 */} -->
